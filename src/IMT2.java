...
public class IMT2 {
    ...
    //quick preferences
    final boolean LIVEGENERATION = true; //sets whether the array is generated on the fly or before the program begins.
   	final int MAXPHRASES = 10000000; //if live generation is on, this sets the max number of phrases.   
    int livegenPhraseInterval = 1000;
	...
	final int MODE = 0; //0- Smart Method, 1- Random Non-shift Method, 2- Random Shift Method
	
	final String VERSION = "1.4"; //version number
    ...
    public void fillArray() {
        ...
        if(LIVEGENERATION){
        	numberOfRows = livegenPhraseInterval; //intervals
        }
        else{
        	numberOfRows = PHRASES;
        }
...
double totalWords = 0;
    ...
    public void masterClass() throws Exception {
        ...
        int percentP = 0;
        while(LIVEGENERATION && createdPhrases < MAXPHRASES){
            	
            	createdPhrases+=livegenPhraseInterval;
            	percentP = (createdPhrases/MAXPHRASES)*100;
            	fillArray();
            	logEnglishWords();
            	//System.out.print("\b\b\b" +(int)percentP +"%");
            	System.out.print("\b\b\b\b" + (int)((createdPhrases*1.0/MAXPHRASES)*100)+"%");
        }
